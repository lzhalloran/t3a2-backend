...
describe("Delete friend route...", () => {
  ...
  it("can delete a friend with valid JWT, friend's user name", async () => {
    const response = await request(app)
    ...
    expect(response.statusCode).toEqual(200);
    expect(response.body).toHaveProperty("message");
    expect(response.body.message).toEqual("Friend deleted successfully");
    encryptedUser1JWT = response.body.jwt;

    requestingUser = await User.findOne({ username: "user1" });
    rejectingUser = await User.findOne({ username: "user2" });
    expect(requestingUser).toHaveProperty("friends");
    expect(rejectingUser).toHaveProperty("friends");
    expect(requestingUser.friends.length).toEqual(0);
    expect(rejectingUser.friends.length).toEqual(0);
  });

  it("error code on invalid JWT", async () => {
    const response = await request(app)
      .delete("/friends/user1")
      .set("jwt", encryptedUser1JWT + 1)
      .send({});
    expect(response.statusCode).toEqual(500);
    expect(response.body).toHaveProperty("error");
  ...
  it("error code when trying to delete a non-friend", async () => {
    const response = await request(app)
      .delete("/friends/user2")
      .set("jwt", encryptedUser1JWT)
      .send({});
